...
description: "Optimize your pidgn application for production with compile-time routing, backend selection, response compression, buffer sizing, and connection tuning."
---

import { Tabs, TabItem, Steps } from '@astrojs/starlight/components';

pidgn is designed for performance from the ground up. Routes are resolved at compile time, middleware chains are inlined by the compiler, and the I/O layer is pluggable. This guide covers the knobs you can turn to get the most out of your deployment.

## Compile-time routing

One of pidgn's most significant performance advantages is that route definitions, pattern matching, and middleware composition are all resolved at `comptime`.
...
<Tabs>
<TabItem label="pidgn (thread pool)">
...
| Setting | Default | Guidance |
|---------|---------|----------|
| `max_connections` | 1024 | Bounded queue capacity (pidgn backend). Size this to your expected peak concurrent connections. |
...
For the native pidgn backend, the `worker_threads` setting controls how many OS threads handle connections:
...
Use this checklist when preparing a pidgn application for production:

| Area | Action | Impact |
|------|--------|--------|
| Build mode | Use `-Doptimize=ReleaseFast` | Enables inlining of comptime middleware chains |
| Backend | Evaluate pidgn vs libhv for your workload | Platform-native I/O can improve throughput |
