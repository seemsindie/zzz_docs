...
pidgn_db provides a `Transaction` API with begin/commit/rollback, automatic nested transactions via savepoints, configurable isolation levels, and a convenience `run` function that handles commit/rollback lifecycle for you.

## Basic transactions

The `Transaction` type is parameterized by backend, just like other pidgn_db types. All methods are static functions that operate on a connection.

```zig
const pidgn_db = @import("pidgn_db");
const sqlite = pidgn_db.sqlite;
const Txn = pidgn_db.Transaction(sqlite);
```
...
```zig
const Txn = pidgn_db.Transaction(sqlite);
const Conn = pidgn_db.Connection(sqlite);
...
When you call `begin` on a connection that is already in a transaction, pidgn_db creates a savepoint instead of a real transaction. This allows nesting without database-level conflicts.
