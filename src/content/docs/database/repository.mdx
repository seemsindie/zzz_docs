---
title: "Repository"
description: "Type-safe CRUD operations with the pidgn_db repository pattern."
---
...
var repo = pidgn_db.PgRepo.init(&pool);
```
</TabItem>
</Tabs>
...
defer pidgn_db.freeOne(User, &updated, allocator);
```
...
```zig
// Fetch all users
const query = pidgn_db.Query(User).init().orderBy("id", .asc);
const users = try repo.all(User, query, allocator);
defer pidgn_db.freeAll(User, users, allocator);
...
defer {
    for (posts_by_user) |posts| {
        for (posts) |*post| {
            var p = post.*;
            pidgn_db.freeOne(Post, &p, allocator);
        }
...
| ----------- | ------------------------------------------- |
| Single struct (`insert`, `get`, `one`, `update`) | `pidgn_db.freeOne(T, &item, allocator)` |
| Slice of structs (`all`, `rawAll`)               | `pidgn_db.freeAll(T, items, allocator)` |
...
This example from the pidgn example application shows a complete CRUD workflow in a web handler:
...
fn dbDemo(ctx: *pidgn.Context) !void {
    ...
    const query = pidgn_db.Query(DemoUser).init().orderBy("id", .desc);
