...
Fetches `pidgn_db` from GitHub and pins it to a specific commit. The `hash` is a content-addressable digest that Zig verifies on fetch.

```zig
// build.zig.zon
.dependencies = .{
    .pidgn_db = .{
        .url = "git+https://github.com/seemsindie/pidgn_db#964b707fbfdea26f75f5fee500317c2cf22c61ac",
        .hash = "pidgn_db-0.3.1-beta.1-0vqMpvi2mQBpxrPOrjYILyrNFRXRFHMHuEdoC0SvQ6rK",
    },
},
```

Replace the commit SHA after `#` with whatever you want to pin to; the hash comes from Zig's package cache.
...
Paste the `<new>` value back into `build.zig.zon`. The workspace ships a helper script `zhash` that automates this — see [Workspace tooling](/getting-started/project-structure/#workspace-tooling).
