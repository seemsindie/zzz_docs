---
title: "Query Builder"
description: "Compose database queries with a fluent, type-safe API in pidgn_db."
---
...
```zig
const base = pidgn_db.Query(User).init()
    .where("name", .eq, "Alice");

const pagination = pidgn_db.Query(User).init()
    .limit(10)
    .offset(20);
...
```zig
fn paginated(comptime T: type, page: u32, per_page: u32) pidgn_db.Query(T) {
    return pidgn_db.Query(T).init()
        .limit(per_page)
        .offset((page - 1) * per_page);
}
