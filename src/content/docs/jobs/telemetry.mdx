---
title: "Job Telemetry"
description: "Monitor pidgn_jobs with event-driven telemetry hooks for job lifecycle events including enqueue, start, completion, failure, and discard."
---

import { Steps } from '@astrojs/starlight/components';

pidgn_jobs provides an event-driven telemetry system that lets you observe job lifecycle events in real time. Use it to log activity, collect metrics, send alerts, or build dashboards.
...
2. **Define a handler function**
    ...
    fn telemetryHandler(event: pidgn_jobs.Event) void {
        switch (event) {
            .job_enqueued => |job| {
...
fn statsHandler(event: pidgn_jobs.Event) void {
    switch (event) {
...
## Example: real-world usage from pidgn_example_app
