...
description: Use pidgn server to run your application with automatic rebuilds and restarts during development.
---
...
For instant browser refresh during development, combine `pidgn server` with [live reload](/frontend/live-reload/). The dev server handles Zig rebuilds, while live reload pushes changes to the browser over a WebSocket.

If your project uses the [asset pipeline](/frontend/asset-pipeline/), run `pidgn assets watch` in a separate terminal alongside the dev server so that CSS and JavaScript changes are also picked up:
...
- [Live Reload](/frontend/live-reload/) for automatic browser refresh
- [Asset Pipeline](/frontend/asset-pipeline/) for bundled and fingerprinted assets
