...
For instant browser refresh during development, combine `zzz server` with [live reload](/frontend/live-reload/). The dev server handles Zig rebuilds, while live reload pushes changes to the browser over a WebSocket.

If your project uses the [asset pipeline](/frontend/asset-pipeline/), run `zzz assets watch` in a separate terminal alongside the dev server so that CSS and JavaScript changes are also picked up:
