...
description: Create, run, and roll back database migrations using the pidgn migrate command.
---
...
```bash
pidgn gen model Post title:string body:text published:boolean
...
2. Run all pending migrations:
   ...
   This is equivalent to `pidgn migrate up`. The CLI runs `zig build run -- --migrate` under the hood, which builds your application and executes the migration runner.
</Steps>
...
The `pidgn migrate` command does not connect to the database directly. Instead, it spawns `zig build run` with a special flag:
