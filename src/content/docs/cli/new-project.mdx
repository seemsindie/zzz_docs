...
description: Use pidgn new to scaffold a complete pidgn web application with routing, configuration, Docker support, and optional database integration.
---

import { Steps } from '@astrojs/starlight/components';

The `pidgn new` command generates a ready-to-run project with a sensible directory structure, environment-based configuration, and optional database and Docker support.

## Usage

```bash
pidgn new <project_name> [options]
```
...
2. **Runtime overrides** -- loaded from `.env` and system environment variables via `pidgn.mergeWithEnv`.
...
```bash
pidgn new my_app --db=postgres
```

Sets `DATABASE_URL=postgres://pidgn:pidgn@localhost:5432/pidgn_dev` and generates a `docker-compose.yml` that includes PostgreSQL 17 and Adminer services.
