...
description: "Overview of testing utilities in the pidgn ecosystem for HTTP, WebSocket, and database testing."
---

import { Tabs, TabItem, Steps } from '@astrojs/starlight/components';

pidgn ships a dedicated testing module that lets you exercise your application without starting a real server or opening network connections. The testing utilities cover three areas:
...
All testing utilities live under `pidgn.testing` and are designed to work with Zig's built-in `test` blocks and `std.testing`.

## What the testing module provides

| Utility | Import | Purpose |
|---|---|---|
| `TestClient` | `pidgn.testing.TestClient` | HTTP request/response testing against your router |
| `TestResponse` | `pidgn.testing.TestResponse` | Response wrapper with assertion methods |
| `RequestBuilder` | `pidgn.testing.RequestBuilder` | Chainable builder for complex requests |
| `CookieJar` | `pidgn.testing.CookieJar` | Automatic cookie persistence across requests |
| `MultipartPart` | `pidgn.testing.MultipartPart` | Multipart/form-data body construction |
| `TestChannel` | `pidgn.testing.TestChannel` | WebSocket channel testing |
| `TestSandbox` | `pidgn_db.testing.TestSandbox` | Transaction-based database isolation |
| `Factory` | `pidgn_db.testing.Factory` | Test record creation with defaults and overrides |

## Setting up tests

pidgn tests run with `zig build test`. No external test runner or server process is needed -- the test client calls your router's handler function directly.
