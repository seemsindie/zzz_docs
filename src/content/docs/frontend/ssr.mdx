...
```zig
fn ssrHandler(ctx: *zzz.Context) !void {
...
The `zzz assets setup --ssr` command generates a worker script like this:
...
- [Assets CLI](/cli/assets-cli/) -- `zzz assets setup --ssr` and build commands
- [Templates](/templates/overview/) -- zzz's built-in template engine for simpler views
