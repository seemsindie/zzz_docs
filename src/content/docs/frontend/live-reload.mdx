...
2. **`liveReloadWs` WebSocket handler** -- maintains the WebSocket connection at `/__zzz/live-reload`.
...
1. **Add the middleware and WebSocket route**
   ...
   const App = zzz.Router.define(.{
       .middleware = &.{
           ...
           zzz.logger,
           zzz.gzipCompress(.{}),
           zzz.liveReload(.{}),  // AFTER gzipCompress
           zzz.staticFiles(.{ .dir = "public", .prefix = "/static" }),
       },
       .routes = &.{
           zzz.Router.get("/", index),
           zzz.Router.websocket("/__zzz/live-reload", .{
               .handler = zzz.liveReloadWs(),
           }),
       },
   });
   ```
...
2. The script opens a WebSocket connection to `/__zzz/live-reload`.
...
|--------|------|---------|-------------|
| `endpoint` | `[]const u8` | `"/__zzz/live-reload"` | WebSocket endpoint path for the reload connection. |
...
const App = zzz.Router.define(.{
    .middleware = &.{
        zzz.errorHandler(.{}),
        zzz.logger,
        if (comptime build_options.live_reload) zzz.liveReload(.{}) else null,
        zzz.staticFiles(.{ .dir = "public", .prefix = "/static" }),
    },
    .routes = &.{
        zzz.Router.get("/", index),
...
- [Assets CLI](/cli/assets-cli/) -- `zzz assets watch` for rebuilding assets on change
