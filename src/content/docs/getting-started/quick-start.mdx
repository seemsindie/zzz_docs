...
For a faster development cycle, use the `zzz server` command to automatically rebuild on source changes, and enable [live reload](/frontend/live-reload/) for instant browser refresh:

```bash
zzz server
```

If your project uses bundled assets, run `zzz assets watch` in a separate terminal. See [Asset Pipeline](/frontend/asset-pipeline/) for setup details.
