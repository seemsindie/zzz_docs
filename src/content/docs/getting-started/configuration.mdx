---
title: "Configuration"
description: "Configuring your pidgn application server and environment variables."
---

import { Tabs, TabItem, Steps } from '@astrojs/starlight/components';

pidgn uses a two-layer configuration system: **comptime defaults** baked in at build time and **runtime overrides** loaded from `.env` files and system environment variables. This page covers the server config options, the environment system, and how they fit together.
...
`pidgn.Env` loads environment variables from `.env` files and the system environment. It is initialized in `main` before the server starts:
...
```zig
var result = try pidgn.configInit(@TypeOf(app_config.config), app_config.config, allocator, .{});
