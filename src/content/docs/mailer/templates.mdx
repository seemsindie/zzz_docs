...
description: "Create templated emails with HTML and plain text bodies, MIME multipart messages, and the pidgn template engine."
---

import { Steps, Tabs, TabItem } from '@astrojs/starlight/components';

pidgn_mailer supports both inline email bodies and template-driven emails using the pidgn template engine. This page covers constructing plain text, HTML, and multipart emails, working with attachments, and using `.pidgn` templates for dynamic content.
...
pidgn_mailer includes a built-in `MimeBuilder` that constructs RFC 5322 / RFC 2045 compliant messages from an `Email` struct. You generally do not need to use this directly -- the adapters call it internally -- but it is available for advanced use cases.

```zig
const pidgn_mailer = @import("pidgn_mailer");
const MimeBuilder = pidgn_mailer.MimeBuilder;
...
const email = Email{
    ...
    .headers = &.{
        .{ .name = "X-Mailer", .value = "pidgn_mailer/0.1.0" },
...
For dynamic emails with variable content, use the pidgn template engine. The `pidgn_template` module is re-exported through `pidgn_mailer.template` for convenience.
...
const pidgn_mailer = @import("pidgn_mailer");
const Email = pidgn_mailer.Email;
const tmpl = pidgn_mailer.template;
...
1. **Create template files**
   ...
   Create `src/templates/welcome.html.pidgn`:
   ```html
   <!DOCTYPE html>
   <html>
   ...
   Create `src/templates/welcome.txt.pidgn`:
   ```
   {{subject}}
...
2. **Embed and render in your mailer module**
   ...
   const HtmlTemplate = tmpl.template(@embedFile("templates/welcome.html.pidgn"));
   const TextTemplate = tmpl.template(@embedFile("templates/welcome.txt.pidgn"));

   pub fn build(allocator: std.mem.Allocator, to: pidgn_mailer.Address, data: anytype) !Email {
       return Email{
           .from = .{ .email = "noreply@example.com", .name = "My App" },
           .to = &.{to},
           .subject = "Welcome",
...
The `pidgn` CLI can scaffold a mailer module for you:
...
For template-based mailers with separate `.pidgn` template files:

```bash
pidgn gen mailer welcome --templates
```

This additionally creates `templates/welcome.html.pidgn` and `templates/welcome.txt.pidgn` files using `@embedFile`.
...
- [Template Syntax](/templates/syntax/) -- full reference for the pidgn template language
