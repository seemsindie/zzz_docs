---
title: "Channels"
description: "Phoenix-style channels with topic-based pub/sub, event routing, join/leave lifecycle, and broadcasting in pidgn."
---
...
1. **Define a channel.** Create a `ChannelDef` with a topic pattern, join handler, and event handlers.
   ...
   fn handleNewMsg(socket: *pidgn.Socket, topic: []const u8, _: []const u8, payload: []const u8) void {
       // Broadcast the message to all subscribers of this topic
       socket.broadcast(topic, "new_msg", payload);
   }

   const room_channel: pidgn.ChannelDef = .{
       .topic_pattern = "room:*",
       .join = &roomJoin,
       .leave = &roomLeave,
       .handlers = &.{
...
3. **Connect from the client.** Use the [pidgn.js client](/websockets/pidgn-js-client/) to connect and join the topic.
...
```zig
fn handleTyping(socket: *pidgn.Socket, topic: []const u8, _: []const u8, payload: []const u8) void {
...
```zig
fn handleWhisper(socket: *pidgn.Socket, topic: []const u8, _: []const u8, payload: []const u8) void {
...
- [pidgn.js Client](/websockets/pidgn-js-client/) -- connect to channels from the browser
