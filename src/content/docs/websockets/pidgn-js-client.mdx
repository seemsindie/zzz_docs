---
title: "pidgn.js Client"
description: "Lightweight JavaScript client library for connecting to pidgn WebSocket endpoints and channels from the browser, with auto-reconnect, heartbeats, and presence."
---

import { Tabs, TabItem, Steps } from '@astrojs/starlight/components';

pidgn.js is a lightweight JavaScript client library bundled with the pidgn framework. It provides two connection modes: a low-level WebSocket wrapper with auto-reconnect, and a higher-level channel socket for topic-based pub/sub. The library also includes a fetch wrapper with automatic CSRF token handling.

## Serving pidgn.js

The pidgn.js file is embedded in the framework binary and served via the `pidgnJs` middleware. Add it to your middleware stack and it will serve the JavaScript file at a configurable path.
...
1. **Add the middleware to your pipeline.**
   ...
   const pipeline = pidgn.Pipeline.define(&.{
       pidgn.pidgnJs(.{}),  // Serves at /__pidgn/pidgn.js by default
       // ... other middleware
   });
   ```
...
2. **Include the script in your HTML.**
   ...
   ```html
   <script src="/__pidgn/pidgn.js"></script>
   ```

</Steps>

### `PidgnJsConfig` options

| Field | Type | Default | Description |
|---|---|---|---|
| `path` | `[]const u8` | `"/__pidgn/pidgn.js"` | URL path to serve the JavaScript file at |
| `max_age` | `u32` | `86400` | `Cache-Control` max-age in seconds (default: 1 day) |

To customize the path:

```zig
pidgn.pidgnJs(.{ .path = "/assets/pidgn.js", .max_age = 3600 }),
```
...
Use `Pidgn.connect` for a raw WebSocket connection with auto-reconnect and exponential backoff. This connects to endpoints created with `Router.ws`.

```javascript
const ws = Pidgn.connect("ws://localhost:9000/ws/echo", {
...
When the connection drops unexpectedly, `Pidgn.connect` automatically reconnects with exponential backoff:
...
Use `Pidgn.socket` for topic-based pub/sub communication. This connects to endpoints created with `Router.channel`.

### Connecting

```javascript
const socket = Pidgn.socket("ws://localhost:9000/socket", {
...
`Pidgn.fetch` is a thin wrapper around the native `fetch()` API that automatically attaches CSRF tokens for non-GET requests.

```javascript
// GET request (no CSRF needed)
const response = await Pidgn.fetch("/api/users");

// POST with JSON body
const result = await Pidgn.fetch("/api/users", {
  method: "POST",
...
`Pidgn.formSubmit` submits a form via AJAX using `FormData`, with automatic CSRF handling:

```javascript
// By element reference
const form = document.getElementById("my-form");
const response = await Pidgn.formSubmit(form);

// By CSS selector
const response = await Pidgn.formSubmit("#my-form");
```
