---
title: "Rate Limiting"
description: "Configure the pidgn rate limiting middleware to throttle requests using a token bucket algorithm, with per-route and global strategies."
---
...
const routes = pidgn.Router.scope("/api", &.{
    pidgn.rateLimit(.{ .max_requests = 10, .window_seconds = 60 }),
}, &.{
    pidgn.Router.get("/data", dataHandler),
    pidgn.Router.post("/submit", submitHandler),
});
```
...
const routes =
    ...
    pidgn.Router.scope("/auth", &.{
        pidgn.rateLimit(.{ .max_requests = 5, .window_seconds = 300 }),
    }, &.{
        pidgn.Router.post("/login", loginHandler),
        pidgn.Router.post("/register", registerHandler),
    })
    // More generous limit on general API
    ++ pidgn.Router.scope("/api", &.{
        pidgn.rateLimit(.{ .max_requests = 100, .window_seconds = 60 }),
    }, &.{
        pidgn.Router.get("/users", usersHandler),
        pidgn.Router.get("/posts", postsHandler),
    });
```
...
const App = pidgn.Router.define(.{
    .middleware = &.{
        pidgn.errorHandler(.{}),
        pidgn.logger,
        pidgn.rateLimit(.{ .max_requests = 120, .window_seconds = 60 }),
    },
    .routes = routes,
});
```
...
const routes =
    ...
    ++ pidgn.Router.scope("/api", &.{
        pidgn.rateLimit(.{ .max_requests = 60, .window_seconds = 60 }),
