...
description: "Configure bearer token and HTTP basic authentication middleware in zzz to protect routes and extract credentials."
---
...
```zig
const admin_routes = zzz.Router.scope("/admin", &.{
    zzz.basicAuth(.{ .required = true, .realm = "Admin" }),
}, &.{
    zzz.Router.get("/", adminDashboard),
    zzz.Router.get("/users", adminUsers),
});
```
</TabItem>
</Tabs>
