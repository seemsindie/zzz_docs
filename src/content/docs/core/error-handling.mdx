...
const App = pidgn.Router.define(.{
    .middleware = &.{
        pidgn.errorHandler(.{}),   // catches all errors below
        pidgn.logger,
        pidgn.bodyParser,
    },
    .routes = &.{
...
```zig
fn errorHandler(ctx: *pidgn.Context) !void {
    ctx.next() catch |err| {
...
```zig
fn riskyHandler(ctx: *pidgn.Context) !void {
    _ = ctx;
...
.middleware = &.{
    pidgn.errorHandler(.{ .show_details = true }),
    pidgn.logger,    // sees the 500 set by errorHandler
    pidgn.bodyParser,
},
```
...
```zig
fn customErrorHandler(ctx: *pidgn.Context) !void {
    ctx.next() catch |err| {
