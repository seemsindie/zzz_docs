...
Add `staticFiles` to your middleware pipeline. For fingerprinted assets with cache-busting URLs, see the [Asset Pipeline](/frontend/asset-pipeline/) instead.
...
```zig
fn downloadReport(ctx: *pidgn.Context) !void {
    ctx.sendFile("reports/monthly.pdf", null); // auto-detect MIME
}

fn downloadCsv(ctx: *pidgn.Context) !void {
...
- [Asset Pipeline](/frontend/asset-pipeline/) -- fingerprinted assets with cache-busting URLs
- [Response Cache](/core/response-cache/) -- cache GET responses in memory
