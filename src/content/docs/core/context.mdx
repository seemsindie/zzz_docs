...
fn search(ctx: *pidgn.Context) !void {
...
pidgn uses a `StatusCode` enum. Common values:
...
pidgn includes a compile-time template engine. The Context provides several render methods:
...
```zig
fn myMiddleware(ctx: *pidgn.Context) !void {
    // pre-processing
    try ctx.next();
